...
const (
	...
	DefaultEnvoyDNSPrefix = ir.DefaultProxyName
...
type Configuration struct {
	...
	// DNSName defines the DNS Name.
	DNSName string

	// EnvoyGatewayDNSPrefix defines the DNS prefix for Envoy Gateway.
	// This should be the name of the Envoy Gateway service when using
	// the Kubernetes provider.
	EnvoyGatewayDNSPrefix string

	// EnvoyDNSPrefix defines the DNS prefix for Envoy. This should be the
	// name of the Envoy service when using the Kubernetes provider.
...
const (
	// ProviderTypeEnvoyGateway defines the "EnvoyGateway" provider. EnvoyGateway
	// implements a self-signed CA.
...
// Envoy Gateway & Envoy returning them as a *Certificates struct or error if encountered.
func GenerateCerts(certCfg *Configuration, egCfg *v1alpha1.EnvoyGateway) (*Certificates, error) {
	// Check if the certCfg is not provided, then default.
	if certCfg == nil {
		certCfg = &Configuration{}
	}

	if certCfg.Provider == nil {
		certCfg.Provider = &CertProvider{Type: ProviderTypeEnvoyGateway}
	}
	...
	case ProviderTypeEnvoyGateway:
		now := time.Now()
		expiry := now.Add(24 * time.Duration(uint32OrDefault(certCfg.Lifetime, DefaultCertificateLifetime)) * time.Hour)
		caCertPEM, caKeyPEM, err := newCA("Envoy Gateway", expiry)
		if err != nil {
			return nil, err
		}
		...
		case v1alpha1.ProviderTypeKubernetes:
			egDNSNames = kubeServiceNames(stringOrDefault(certCfg.EnvoyGatewayDNSPrefix, DefaultEnvoyGatewayDNSPrefix),
				stringOrDefault(certCfg.Namespace, DefaultNamespace),
				stringOrDefault(certCfg.DNSName, DefaultDNSSuffix))
			envoyDNSNames = kubeServiceNames(stringOrDefault(certCfg.EnvoyDNSPrefix, DefaultEnvoyDNSPrefix),
				stringOrDefault(certCfg.Namespace, DefaultNamespace),
				stringOrDefault(certCfg.DNSName, DefaultDNSSuffix))
		default:
		...
		egCertReq := &certificateRequest{
			...
			caKeyPEM:   caKeyPEM,
			expiry:     expiry,
			commonName: stringOrDefault(certCfg.EnvoyGatewayDNSPrefix, DefaultEnvoyGatewayDNSPrefix),
			altNames:   egDNSNames,
		}
		...
		envoyCertReq := &certificateRequest{
			...
			caKeyPEM:   caKeyPEM,
			expiry:     expiry,
			commonName: stringOrDefault(certCfg.EnvoyDNSPrefix, DefaultEnvoyDNSPrefix),
	...
	default:
		// EnvoyGateway is the only supported certificate provider.
