...
var (
	// closeChan is the close channel.
	closeChan = make(chan bool)

	// jwtPubKeyRefreshInterval is the interval to fetch the JWKS URI for the jwks public key.
	jwtPubKeyRefreshInterval = 30 * time.Minute

	// jwksUriChannel channel for making JWKS URI requests asynchronous.
...
// Verifier verifies a JSON Web Key Set.
...
func NewVerifier(refreshInterval, refreshIntervalOnFailure, retryInterval time.Duration) *Verifier {
	src := NewRemoteSource()
	cli := newClient()
	ret := &Verifier{
